...
class HomeActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val idcard=intent.getStringExtra("id").toString()
        val contact=intent.getStringExtra("contact").toString()
        val userid=intent.getStringExtra("userid").toString()
        Log.d(ContentValues.TAG,"myname is $name myid is $idcard myconnect is $contact userid is$userid")
        val bundle=Bundle()
        bundle.putString("name",name)
        bundle.putString("idcard",idcard)
        bundle.putString("contact",contact)
        bundle.putString("userid",userid)
        mTab03.arguments=bundle
